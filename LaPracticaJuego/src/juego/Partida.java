...
public class Partida {
	...
	public static void mostrarPreguntasCpu(Jugador cpu) {
		...
		if (pregunta == Constantes.ARCHIVO_PREGUNTAS_MATES) {
			PreguntasMates mates = new PreguntasMates();
			mates.generarOperacion();

			mates.respuestaMatesCPU();
			System.out.println("La respuesta de la CPU es: " + mates.respuestaMatesCPU());
			System.out.println("Correcto!!!");
			cpu.anyadirPuntos();

		} else if (pregunta == Constantes.ARCHIVO_PREGUNTAS_LETRAS) {
			PreguntasLetras letras = new PreguntasLetras();
			System.out.println(letras.getPalabraOculta());
			System.out.println("Respuesta de la cpu: " + letras.respuestaLetrasCpu());
			System.out.println("La cpu no ha adivinado la palabra");
		} else if (pregunta == Constantes.ARCHIVO_PREGUNTAS_INGLES){
			PreguntasIngles ingles = new PreguntasIngles(null, aleatorio, null);
			ingles.mostrarPregunta();
			ingles.comprobarRespuestaCPU();

			if (ingles.verificarRespuesta(ingles.comprobarRespuestaCPU())) {
